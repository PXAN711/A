...
const EMPTY_ROOM_TTL = 30000;    // 房间无真人后保留多久
...
function createRoom(id) {
  ...
  const socketSeat = new Map();   // 在座真人 socketId -> seat
  const guests = new Map();       // 房内未入座 socketId -> name
  ...
  function clearAllBotTimers() {
    for (const s of Object.keys(game.botTimers)) {
      if (game.botTimers[s]) { clearTimeout(game.botTimers[s]); game.botTimers[s] = null; }
    }
  }
  function addLog(msg) {
  ...
  function botSchedule(seat, ms) {
    ...
    game.botTimers[seat] = setTimeout(() => {
      ...
      try { botAct(seat); } catch (e) { console.log('bot error', id, seat, e); }
    }, ms);
  }
  ...
  function botAct(seat) {
    ...
    if (a.mode === 'rob') return onRobAct(seat, 'hu');
  ...
  function maybeResolvePending() {
    ...
    const huDecided = ask.hu.every(s => ask.resp[s]);
    ...
    if (game.current === seat) {
      ...
      for (let q = 0; q < 4; q++) {
        ...
        for (const m of o.melds) {
          if (q !== seat && m.type === 'angang') continue;
  ...
  function broadcast() {
    ...
    for (const [gid] of guests) io.to(gid).emit('room', lobbyView());
    driveBots();
    rooms._refresh();
  }

  // ---------------- 入座/离座/进出房 ----------------
  // 进入房间（大厅点房间卡片 / 创建房间）。同名离线玩家自动回座
  function enter(socket, name) {
    socket.join(roomName);
    cancelDestroy();
    const existSeat = game.players.findIndex(p => p && p.name === name);
    if (existSeat >= 0) {
      const p = game.players[existSeat];
      if (p.isBot) { socket.leave(roomName); return socket.emit('error_msg', '该 ID 已被人机占用，换一个'); }
      if (p.online && p.socketId !== socket.id) { socket.leave(roomName); return socket.emit('error_msg', '该 ID 已被占用，换一个'); }
    ...
    guests.set(socket.id, name);
    socket.emit('room', lobbyView());
    rooms._refresh();
  }
  // 房内选座
  function doJoin(socket, name, seat) {
    name = (name || '').trim().slice(0, 12) || guests.get(socket.id);
  ...
  function leaveSeat(socket) {
    const seat = socketSeat.get(socket.id);
    const name = seat != null && game.players[seat] ? game.players[seat].name : guests.get(socket.id);
    if (seat == null) return socket.emit('room', lobbyView());
    ...
    if (name) guests.set(socket.id, name);
    addLog(`${name} 回到选座`);
    broadcast();
    socket.emit('room', lobbyView());
  }
  function addBot(seat) {
    if (game.phase !== 'idle' && game.phase !== 'settle') return;
    let target = Number.isInteger(seat) ? seat : game.players.findIndex(p => !p);
    if (target < 0 || target > 3 || game.players[target]) return;
    ...
    game.players[target] = newPlayer(target, name, `__bot__${id}S${target}`, true);
    addLog(`${name} 入座（电脑）`);
    broadcast();
  }
  ...
  // 游戏内事件统一入口
  function handle(socket, action, data = {}) {
    const seat = socketSeat.get(socket.id);
    const p = game.players[seat];
    switch (action) {
      case 'ready':
        ...
        return;
      case 'addBot': {
        if (game.phase !== 'idle' && game.phase !== 'settle') return socket.emit('error_msg', '对局进行中，下一局才能添加人机');
        addBot(data.seat); return;
      }
      case 'removeBot': {
        if (game.phase !== 'idle' && game.phase !== 'settle') return socket.emit('error_msg', '对局进行中不能移除人机');
        removeBot(data.seat); return;
      }
      case 'exchange': {
      ...
      case 'lack': {
        ...
        if (!SUITS.includes(data.suit)) return err(seat, '缺门无效');
        p.lack = data.suit;
        if (game.players.every(x => x && x.lack)) beginPlay();
        else broadcast();
        return;
      }
      case 'discard': return doDiscard(seat, data.tile);
      case 'selfWin': return onSelfWin(seat);
      case 'selfKong': return onSelfKong(seat, data.type, data.tile);
      case 'act': return onAct(seat, data.type);
      case 'chat': {
        if (!p) return;
        const msg = ('' + (data.msg || '')).trim().slice(0, 60);
        if (msg) io.to(roomName).emit('chat', { name: p.name, msg, t: Date.now() });
        return;
      }
    }
  }
  ...
  function onDisconnect(socket) {
    ...
    const seat = socketSeat.get(socket.id);
    socketSeat.delete(socket.id);
    if (seat == null) { scheduleDestroy(); return; }
    const p = game.players[seat]; if (!p) { scheduleDestroy(); return; }
  ...
  function leaveRoom(socket) {
    ...
    if (seat != null) {
      const p = game.players[seat];
      if (p && game.phase !== 'idle' && game.phase !== 'settle') {
        socket.emit('error_msg', '对局进行中不能离开，关闭页面将由系统托管');
        return false;
      }
      if (p) { botClear(seat); game.players[seat] = null; addLog(`${p.name} 离开房间`); }
      socketSeat.delete(socket.id);
    }
    guests.delete(socket.id);
  ...
  function onlineHumans() {
    let n = guests.size;
    for (const p of game.players) if (p && !p.isBot && p.online) n++;
    return n;
  }
  function scheduleDestroy() {
    if (onlineHumans() > 0) return;
    cancelDestroy();
    destroyTimer = setTimeout(() => api.destroy(), EMPTY_ROOM_TTL);
  }
  function cancelDestroy() { if (destroyTimer) { clearTimeout(destroyTimer); destroyTimer = null; } }
  function summary() {
    return {
      id, phase: game.phase, round: game.round,
      n: game.players.filter(p => p).length,
      bots: game.players.filter(p => p && p.isBot).length,
      waiting: game.phase === 'idle' || game.phase === 'settle',
    };
  }
  function destroy() {
    cancelDestroy();
    clearAllTimers(); clearAllBotTimers();
    rooms.delete(id);
    rooms._refresh();
  }
  const api = {
    id, roomName, enter, doJoin, leaveSeat, leaveRoom, handle, onDisconnect, destroy,
    cancelDestroy, scheduleDestroy, summary,
  };
  return api;
}

// ================= 房间大厅（服务端顶层） =================
const rooms = new Map();   // id -> room api
...
io.on('connection', (socket) => {
  ...
  function currentRoom() { return socket.data.roomId ? rooms.get(socket.data.roomId) : null; }
  function exitRoom() {
    const room = currentRoom();
    if (room && room.leaveRoom(socket)) socket.data.roomId = null;
  }
  ...
  socket.on('createRoom', () => {
    if (!socket.data.name) return socket.emit('error_msg', '请先输入 ID');
    exitRoom();
    const id = newRoomId();
    const room = createRoom(id);
    rooms.set(id, room);
    socket.data.roomId = id;
    room.enter(socket, socket.data.name);
    rooms._refresh();
  });
  ...
  socket.on('join', (data = {}) => { const r = currentRoom(); if (r) r.doJoin(socket, socket.data.name, data.seat); });
  socket.on('leaveSeat', () => { const r = currentRoom(); if (r) r.leaveSeat(socket); });
  socket.on('leaveRoom', () => {
    if (exitRoom()) socket.emit('rooms', roomList());
  });

  ['ready', 'addBot', 'removeBot', 'exchange', 'lack', 'discard', 'selfWin', 'selfKong', 'act', 'chat']
    .forEach(action => socket.on(action, data => { const r = currentRoom(); if (r) r.handle(socket, action, data || {}); }));
